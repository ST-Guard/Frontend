...
function buscarDados() {
  ...
  // pegar dados do bucket financeiro
  var BUCKET = "";
...
function atualizarSelo(id, valor, positivoVerde = true) {
  ...
  elemento.classList.add(
    positivo === positivoVerde ? "badge-kpi-verde" : "badge-kpi-vermelho",
  );
}
...
function aplicarHistoricoMensal(historicoMensal, projecoes = []) {
  ...
  const tresMesesProjecao = Array.isArray(projecoes)
    ? projecoes.slice(0, 3)
    : [];
  const ultimoCustoHistorico =
    Number(historicoMensal[historicoMensal.length - 1]?.custo || 0) / 1000;
  const ultimaReceitaHistorica =
    Number(historicoMensal[historicoMensal.length - 1]?.receita || 0) / 1000;

  HISTORICO_MESES = historicoMensal.map((item) => formatarMesGrafico(item.mes));
  custoHistorico = historicoMensal.map(
    (item) => Number(item.custo || 0) / 1000,
  );
  receitaHistorica = historicoMensal.map(
    (item) => Number(item.receita || 0) / 1000,
  );
  historicoRoi = historicoMensal.map((item) => Number(item.roi || 0));

  todosRotulos = [
    ...HISTORICO_MESES,
    ...tresMesesProjecao.map((item) => formatarMesGrafico(item.mes)),
  ];
  todosCustos = [...custoHistorico, ...tresMesesProjecao.map(() => null)];
  todasReceitas = [...receitaHistorica, ...tresMesesProjecao.map(() => null)];
  prevCustoLn = [
    ...HISTORICO_MESES.slice(0, -1).map(() => null),
    ultimoCustoHistorico,
    ...tresMesesProjecao.map((item) => Number(item.custo_previsto || 0) / 1000),
  ];
  prevReceitaLn = [
    ...HISTORICO_MESES.slice(0, -1).map(() => null),
    ultimaReceitaHistorica,
    ...tresMesesProjecao.map(
      (item) => Number(item.receita_prevista || 0) / 1000,
    ),
  ];

  definirPeriodoGrafico(
    periodoGraficoLinha,
    document.querySelector(".btn-periodo.active:not(.botao-periodo-roi)"),
  );
}
...
function plotarDados(dadosS3) {
  ...
  atualizarTexto(
    "kpi-rev-delta",
    formatarVariacaoMoeda(faturamento.DELTA_FATURAMENTO),
  );
  ...
  budgetApprovedValue = calcularOrcamentoAprovado(
    budgetRegressionValue,
    budgetMarginPct,
  );
  ...
  // GRAFICOS
  projecoesPreditivas = Array.isArray(dadosS3.PROJECOES)
    ? dadosS3.PROJECOES.slice(0, 6)
    : [];
  ...
  if (document.getElementById("modal-preditivo")?.classList.contains("active"))
    renderizarPreditivo();
}
  ...
  document
    ...
    .forEach((btn) =>
      btn.classList.toggle(
        "active",
        Number(btn.textContent.replace("M", "")) === mesesForecast,
      ),
    );
  renderizarPreditivo();
}
...
function renderizarPreditivo() {
  ...
  atualizarTexto(
    "model-mae-badge_custo",
    `MAE CUSTO ${formatarMoeda(maeCusto)}`,
  );
  atualizarTexto(
    "model-mae-badge_receita",
    `MAE RECEITA ${formatarMoeda(maeReceita)}`,
  );

  const htmlPreditivo = projecoes.length
    ? projecoes
        .map((item, i) => {
          const confiancaCusto = obterNumeroS3(item, [
            "confianca_custo",
            "CONFIANCA_CUSTO",
            "confianca",
            "CONFIANCA",
          ]);
          const confiancaReceita = obterNumeroS3(item, [
            "confianca_receita",
            "CONFIANCA_RECEITA",
            "confianca",
            "CONFIANCA",
  ...
  const labels = [
    ...historicoLabels,
    ...projecoes.map((item) => formatarMesGrafico(item.mes)),
  ];
    ...
    datasets: [
      {
        ...
        data: custosHistoricos,
        borderColor: "#2C5D86",
        backgroundColor: "rgba(44,93,134,0.07)",
        borderWidth: 2,
        pointRadius: 3,
        tension: 0.35,
        fill: true,
      ...
      },
      {
        ...
        data: receitasHistoricas,
        borderColor: "#F5CC4D",
        backgroundColor: "rgba(245,204,77,0.06)",
        borderWidth: 2,
        pointRadius: 3,
        tension: 0.35,
        fill: true,
  ...
  const opcoesGrafico = {
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          label: (ctx) =>
            `${ctx.dataset.label}: ${formatarMoeda(Number(ctx.raw || 0) * 1000)}`,
        },
      },
    },
    scales: {
      y: {
        ticks: { color: "#6B7E91", callback: (valor) => `${valor}k` },
        grid: { color: "rgba(216,227,238,0.8)" },
      },
...
function atualizarPainelOrcamento() {
  ...
  atualizarTexto(
    "kpi-cost-budget-ref",
    formatarMoeda(budgetCustoCorrenteValue * 1.05),
  );
  ...
  atualizarTexto(
    "kpi-budget-margin-label",
    budgetMarginPct.toLocaleString("pt-BR", { maximumFractionDigits: 1 }),
  );
  atualizarTexto(
    "budget-margin-display",
    budgetMarginPct.toLocaleString("pt-BR", { maximumFractionDigits: 1 }),
  );
...
function construirGraficoRoi(rotulos, historico) {
  ...
  instanciaGraficoRoi = new Chart(
    document.getElementById("roiChart").getContext("2d"),
    {
      type: "line",
      data: {
        labels: rotulos,
...
function definirPeriodoRoi(periodo, botao) {
  document
    .querySelectorAll(".botao-periodo-roi")
  ...
  if (periodo === 1) inicio = Math.max(HISTORICO_MESES.length - 1, 0);
    ...
    const ultimoAno =
      HISTORICO_MESES[HISTORICO_MESES.length - 1]?.split("/")[1];
    const indiceYtd = HISTORICO_MESES.findIndex(
      (mes) => mes.endsWith(`/${ultimoAno}`) && mes.startsWith("Jan"),
    );
    inicio = indiceYtd >= 0 ? indiceYtd : 0;
  }

  construirGraficoRoi(
    HISTORICO_MESES.slice(inicio),
    historicoRoi.slice(inicio),
  );
}
...
function abrirModalRoi() {
  ...
  const botaoAtivo =
    document.querySelector(".botao-periodo-roi.active") ||
    document.querySelectorAll(".botao-periodo-roi")[2];
  const indicePeriodo =
    Array.from(document.querySelectorAll(".botao-periodo-roi")).indexOf(
      botaoAtivo,
    ) + 1;
...
function gerarPrevisoes(quantidade) {
  ...
  for (let i = 0; i < quantidade; i++) {
    ...
    const roiFicticio = (
      ((receitaFicticia - custoFicticio) / custoFicticio) *
      100
    ).toFixed(1);
...
let todosRotulos = [
  ...HISTORICO_MESES,
  ...previsoesL3.map((_, i) => rotuloMes(i)),
];
let todosCustos = [...custoHistorico, ...Array(3).fill(null)];
let todasReceitas = [...receitaHistorica, ...Array(3).fill(null)];
let prevCustoLn = [
  ...custoHistorico.map(() => null),
  custoHistorico[11],
  ...previsoesL3.map((f) => f.custo),
];
let prevReceitaLn = [
  ...receitaHistorica.map(() => null),
  receitaHistorica[11],
  ...previsoesL3.map((f) => f.receita),
];
let historicoRoi = custoHistorico.map((c, i) =>
  parseFloat((((receitaHistorica[i] - c) / c) * 100).toFixed(1)),
);
...
function criarOpcoesGraficoLinha(unidade, formatarValorGrafico) {
  return {
    ...
    scales: {
      y: {
        ticks: {
          callback: (v) => (unidade === "k" ? `${v}k` : formatarMoeda(v)),
          font: { size: 11 },
        },
        grid: { color: "rgba(0,0,0,0.05)" },
      },
...
function construirGraficoLinha(
  rotulos,
  custo,
  receita,
  unidade = "k",
  ...
  if (
    unidade === "k" &&
    custoPrevisto.length === rotulos.length &&
    receitaPrevista.length === rotulos.length
  ) {
    datasets.push(
      {
        label: "Custo previsto",
...
function definirPeriodoGrafico(periodo, botao) {
  ...
  document
    .querySelectorAll(".btn-periodo:not(.botao-periodo-roi)")
    .forEach((b) => b.classList.remove("active"));
  if (botao) botao.classList.add("active");
  ...
  if (periodo === "24h") {
    ...
    const previsoesRotulos = todosRotulos.slice(
      HISTORICO_MESES.length,
      HISTORICO_MESES.length + 3,
    );
    ...
    const custo = [
      ...custoHistorico.slice(inicio),
      ...Array(qtdPrevisoes).fill(null),
    ];
    const receita = [
      ...receitaHistorica.slice(inicio),
      ...Array(qtdPrevisoes).fill(null),
    ];
    const custosPrevistosFuturos =
      qtdPrevisoes > 0 ? prevCustoLn.slice(-qtdPrevisoes) : [];
    const receitasPrevistasFuturas =
      qtdPrevisoes > 0 ? prevReceitaLn.slice(-qtdPrevisoes) : [];
    const custoPrevisto =
      qtdPrevisoes > 0
        ? [
            ...Array(Math.max(historicoVisivel - 1, 0)).fill(null),
            custoHistorico[custoHistorico.length - 1],
            ...custosPrevistosFuturos,
          ]
        : [];
    const receitaPrevista =
      qtdPrevisoes > 0
        ? [
            ...Array(Math.max(historicoVisivel - 1, 0)).fill(null),
            receitaHistorica[receitaHistorica.length - 1],
            ...receitasPrevistasFuturas,
          ]
        : [];
  ...
  if (estadoFiltroGrafico !== "todos")
    alternarGraficoLinha(estadoFiltroGrafico, true);
}
...
function alternarSerieGraficoLinha(indice) {
  ...
  graficoLinha.data.datasets[indice].hidden =
    !graficoLinha.data.datasets[indice].hidden;
  graficoLinha.update("none");
}
...
let dcRois = dcCosts.map((c, i) =>
  parseFloat((((dcRevs[i] - c) / c) * 100).toFixed(1)),
);
...
function definirVisualizacaoBarra(view, btn) {
  barView = view;
  document
    .querySelectorAll(".alternador-vista .btn-alternador")
    .forEach((b) => b.classList.remove("active"));
...
function aplicarBarrasCentrosDados(barrasDatacenter) {
  ...
  dcLabels = barrasDatacenter.map((item) =>
    primeiroValorObjeto(
      item,
      ["datacenter", "DATACENTER", "dc", "DC", "nome", "NOME", "name", "label"],
      "DC",
    ),
  );
  dcCosts = barrasDatacenter.map((item) =>
    Number(
      primeiroValorObjeto(
        item,
        [
  ...
  dcRevs = barrasDatacenter.map((item) =>
    Number(
      primeiroValorObjeto(
        item,
        [
  ...
  dcRois = barrasDatacenter.map((item, i) => {
    ...
    return dcCosts[i] > 0
      ? Number((((dcRevs[i] - dcCosts[i]) / dcCosts[i]) * 100).toFixed(1))
      : 0;
  });
  ...
  document.getElementById("bar-sub").textContent =
    barView === "custo"
      ? "Custo mensal — soma = " +
        formatarMoeda(dcCosts.reduce((soma, valor) => soma + valor, 0))
      : "Receita sobre custo por DC (negativo = prejuízo)";
  construirGraficoBarra();
}
  ...
  barChartInst = new Chart(
    document.getElementById("barChart").getContext("2d"),
    {
      ...
      data: {
        ...
        datasets: [
          {
            label: isCusto ? "Custo mensal" : "ROI (%)",
            data,
...
function normalizarClasseSituacao(status) {
  ...
  if (
    ...
    texto.includes("ofensor") ||
    texto.includes("alto")
  )
    return "critico";
  if (
    texto.includes("alert") ||
    texto.includes("medio") ||
    texto.includes("médio")
  )
    return "alerta";
  return "ativo";
}
...
function aplicarPrincipaisServidores(topServidores) {
  ...
  servidoresRows = topServidores.map((item) => ({
    ...
    dc: primeiroValorObjeto(
      item,
      ["DATACENTER", "datacenter", "DC", "dc"],
      "-",
    ),
    ...
    cost: Number(
      primeiroValorObjeto(
        item,
        ["CUSTO", "custo", "custo_total", "CUSTO_TOTAL", "cost", "valor"],
        0,
      ),
    ),
    energy: Number(
      primeiroValorObjeto(
        item,
        ["CUSTO_ENERGIA", "custo_energia", "ENERGIA", "energia", "energy"],
        0,
      ),
    ),
    st: primeiroValorObjeto(
      item,
      ["STATUS", "status", "statusClasse", "status_classe"],
      "ativo",
    ),
    stl: primeiroValorObjeto(
      item,
      ["STATUS_LABEL", "status_label", "statusLabel", "STATUS", "status"],
      "Ativo",
    ),
  }));
}
...
function aplicarPrincipaisZonas(topZonas) {
  if (!topZonas.length) return;

  zonasRows = topZonas.map((item) => ({
    zona: primeiroValorObjeto(
      item,
      ["ZONA", "zona", "zone", "nome", "name"],
      "-",
    ),
    dc: primeiroValorObjeto(
      item,
    ...
    cost: Number(
      primeiroValorObjeto(
        item,
        ["CUSTO_TOTAL", "custo_total", "CUSTO", "custo", "cost", "valor"],
        0,
      ),
    ),
    energy: Number(
      primeiroValorObjeto(
        item,
        ["ENERGIA", "energia", "CUSTO_ENERGIA", "custo_energia", "energy"],
        0,
      ),
    ),
    st: primeiroValorObjeto(
...
function renderTabelaAtual(animar = true) {
  ...
  const html =
    curView === "servidores" ? renderizarServidores() : renderizarZonas();
  if (!animar) {
    w.innerHTML = html;
    return;
  }
...
function definirVisualizacaoTabela(view, btn) {
  ...
  const botoes = grupoTabela
    ? grupoTabela.querySelectorAll(".btn-alternador")
    : document.querySelectorAll(".btn-alternador");
  botoes.forEach((b) => b.classList.remove("active"));
  if (btn) btn.classList.add("active");
  document.getElementById("table-title").textContent =
    view === "servidores" ? "Top servidores por custo" : "Top zonas por custo";
  document.getElementById("table-sub").textContent =
    view === "servidores"
      ? "Maiores consumidores do orçamento mensal"
      : "Custo consolidado por zona de disponibilidade";
...
async function buscarRelatoriosS3() {
  ...
  lista.innerHTML =
    '<div class="estado-lista-relatorios">Buscando relatórios na S3...</div>';
...
function renderizarListaRelatorios() {
  ...
  lista.innerHTML = relatoriosDisponiveis
    .map(
      (relatorio, indice) => `
...
async function abrirRelatorioS3(relatorio) {
  ...
  corpo.innerHTML =
    '<div class="estado-lista-relatorios">Carregando PDF...</div>';
  modal.classList.add("active");
...
document
  .getElementById("report-full-modal")
...
document
  .getElementById("modal-preditivo")
  .addEventListener("click", function (e) {
